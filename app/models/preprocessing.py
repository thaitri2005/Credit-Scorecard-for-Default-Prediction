...
class DataPreprocessor:
    ...
    def decision_tree_binning(self, X: pd.Series, y: pd.Series, max_leaf_nodes: int = 5) -> List[float]:
        """
    ...
    def calculate_woe_iv(self, df: pd.DataFrame, feature_col: str, target_col: str = 'target') -> Tuple[Dict, float]:
        """
    ...
    def map_loan_status(self, status: str) -> int:
        ...
        Returns:
            0 for good, 1 for bad, None for ambiguous
        """
        bad_statuses = [
            'Charged Off', 
            'Default', 
        ...
        if status in bad_statuses:
            ...
            return None  # Exclude ambiguous cases
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        existing_heavy_missing = [col for col in drop_heavy_missing if col in df_clean.columns]
        ...
        if 'issue_d' in df_clean.columns and 'earliest_cr_line' in df_clean.columns:
            ...
            df_clean['credit_history_length'] = (df_clean['issue_d'] - df_clean['earliest_cr_line']).dt.days / 365
        ...
        if 'loan_amnt' in df_clean.columns and 'annual_inc' in df_clean.columns:
            df_clean['loan_burden'] = df_clean['loan_amnt'] / (df_clean['annual_inc'] + 1)
            
            # Drop original columns
            df_clean = df_clean.drop(columns=['loan_amnt', 'dti'])
    ...
    def _impute_missing_values(self, df: pd.DataFrame):
        ...
        numerical_cols = [
            'total_rev_hi_lim', 'tot_coll_amt', 'tot_cur_bal', 'revol_util',
            'collections_12_mths_ex_med', 'acc_now_delinq', 'total_acc',
            'pub_rec', 'open_acc', 'inq_last_6mths', 'delinq_2yrs',
            'credit_history_length', 'annual_inc', 'mths_since_last_record',
        ...
        if 'emp_length' in df.columns:
            mode_val = df['emp_length'].mode()[0] if not df['emp_length'].mode().empty else 'Unknown'
