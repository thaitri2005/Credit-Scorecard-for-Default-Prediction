...
class LoanApplicationRequest(BaseModel):
    ...
    total_rev_hi_lim: Optional[float] = Field(None, description="Total revolving high limit", ge=0)
    tot_cur_bal: Optional[float] = Field(None, description="Total current balance", ge=0)
    ...
    revol_util: Optional[float] = Field(None, description="Revolving utilization percentage", ge=0, le=100)
...
class BatchPredictionRequest(BaseModel):
    ...
    include_details: bool = Field(False, description="Whether to include detailed results")

class BatchPredictionResponse(BaseModel):
    """Response model for batch predictions"""
    predictions: List[CreditScoreResponse] = Field(..., description="List of predictions")
