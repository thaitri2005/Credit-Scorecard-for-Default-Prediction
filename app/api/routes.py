...
async def predict_credit_score(request: LoanApplicationRequest, model: CreditRiskModel = Depends(get_model)):
...
async def predict_batch_credit_scores(
    ...
    try:
        ...
        if len(request.applications) > 100:  # Limit batch size
            raise HTTPException(status_code=400, detail="Batch size cannot exceed 100 applications")
        ...
        for i, application in enumerate(request.applications):
            try:
                ...
                if not is_valid:
                    ...
                    predictions.append(CreditScoreResponse(
                        credit_score=0.0,
...
async def get_available_features():
    ...
    try:
        ...
        return {
            ...
            "required_features": ["annual_inc", "int_rate", "purpose", "verification_status"],
            "optional_features": ["total_rev_hi_lim", "tot_cur_bal", "loan_burden", "revol_util", "credit_history_length"]
        }
    except Exception as e:
